import { generateMusicFromPrompt, getGeneratedMusics } from '../services/musicService.js';

export async function generateMusic(req, res) {
  try {
    const { prompt } = req.body;
    if (!prompt || prompt.trim() === '') {
      return res.status(400).json({ message: 'O prompt é obrigatório.' });
    }

    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ message: 'Usuário não autenticado.' });

    const newMusic = await generateMusicFromPrompt(prompt, userId);
    // Retorna 202 Accepted porque a geração é assíncrona
    return res.status(202).json(newMusic);
  } catch (error) {
    console.error('Erro em generateMusic:', error);
    return res.status(500).json({ message: error.message || 'Erro ao iniciar a geração de música.' });
  }
}

export async function getMusics(req, res) {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ message: 'Usuário não autenticado.' });

    const musics = await getGeneratedMusics(userId);
    return res.status(200).json(musics);
  } catch (error) {
    console.error('Erro em getMusics:', error);
    return res.status(500).json({ message: 'Erro ao listar músicas.' });
  }
}
